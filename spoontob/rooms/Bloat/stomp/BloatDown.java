...
public final class BloatDown {
    ...
    private static final HashBasedTable<BloatChunk, Direction, Pair<BloatPath, BloatRotation>> chunkTable = HashBasedTable.create();
    ...
    public BloatDown(@Nonnull Client client, @Nonnull WorldPoint sw, @Nonnull Direction dir, @Nonnull BloatChunk chunk) {
        ...
        this.bloatSafespot = new BloatSafespot((Pair<BloatPath, BloatRotation>)chunkTable.get(chunk, dir), () -> {
            int off = (dir != Direction.NORTH && dir != Direction.EAST) ? 0 : 4;
            return Integer.valueOf(Math.max(sw.distanceTo2D(this.destination) - off, 0));
        });
    }
